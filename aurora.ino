  ...
  for(uint16_t y=1; y<inhaleDuration; y++) {
    ...
    for(uint16_t x=0; x<range; x++) {
      ...
      for(uint8_t b=0; b<2; b++) {
        ...
        if(x == 0) {
          mutatePixel(center, x, range, y, inhaleDuration, tarColor, marker);
        }
        // Selects the left pixel of the current range
        else if(b == 0) {
          mutatePixel((center - x), x, range, y, inhaleDuration, tarColor, marker);
        }
